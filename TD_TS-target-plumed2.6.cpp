...
\f[
\begin{split}
  p(s)=p({s}) &= \frac{1}{Z_p} \ \frac{\zeta}{\zeta^2 +  \left({\beta \frac{d F({s})}{d s}}\right)^2}\left( \frac{1-a}{1 \ + \ e^{ \lambda \beta \frac{d^{2}F({s})}{ds^2}}} + a\right) \\
&= p_L(s) S_{a,\lambda}(s) \, .
\end{split} 
\f]

The SCALEFACTOR \f$ \zeta \f$ determines the width of the peaks in the p(s). The optional
keyword LAMBDA \f$ \lambda \f$ (default: 0.1) determines how fast the switching function decays 
and the optional keyword LIMIT \f$ a \f$ (default: 1) is used to specify the scaling
between the probabilities of sampling the minima and the maximum.

An optional keyword PRINT_STRIDE can be used to specify the frequency of 
printing the first derivative, second derivative and switching function grids. In the absence 
...
\warning NOTE: This target distribution works only for one-dimensional cases.
  ...
  bf_norm(1.0)
{
  ...
  {
    log.printf("  TS-target distribution, see and cite ");
    log << plumed.cite(" Debnath, Invernizzi and Parrinello, J. Chem. Theory Comput. 2019  15, 4, 2454-2459 (2019)");
    log << plumed.cite(" Valsson and Parrinello, Phys. Rev. Lett. 113, 090601 (2014) ");
    log.printf("\n");
    log.printf("SCALEFACTOR %f", scale_factor_);
    log.printf("\n");
...
// --------------- initializes quantities at the first iteration ---------------
   if(derv_grid_pntr_==NULL){ 
     ...
     derv_grid_pntr_ = new Grid("grad_fes",args,min,max,nbins,false,false,isperiodic,min,max);
...
// --------------- initializes quantities ---------------
  ...
  if(derv2_grid_pntr_==NULL){
    ...
    std::vector<std::string> max = getFesGridPntr()->getMax();
    std::vector<std::string> args = getFesGridPntr()->getArgNames();
    derv2_grid_pntr_ = new Grid("gradient-order2",args,min,max,nbins,false,false,isperiodic,min,max);
    //derv2_grid_pntr_ = new Grid("gradient-order2",args,min,max,nbins,false,false,true,isperiodic,min,max);
    switch_grid_pntr_ = new Grid("switch",args,min,max,nbins,false,false,isperiodic,min,max);
